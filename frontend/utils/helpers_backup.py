...
def create_samples_table(data, current_page=1, page_size=10, sort_column='data', sort_order='desc'):
    ...
    for sample in paginated_data:
        ...
        if sample_id == 'N/A' or sample_id is None:
            continue
        ...
        # Create detail link - ensure sample_id is valid  
        detail_link = html.Span("No disponible", style={'color': '#6c757d'})
        try:
            if sample_id and str(sample_id).strip() and str(sample_id) != 'N/A':
                sample_id_clean = str(sample_id).strip()
                detail_url = f"/browse/sample/{sample_id_clean}"
            ...
            print(f"Error creating link for sample {sample_id}: {e}")
        
        row = html.Tr([
            html.Td(str(sample_id), style={'padding': '1rem', 'borderBottom': '1px solid #dee2e6', 'fontWeight': '500'}),
            html.Td(str(date_str), style={'padding': '1rem', 'borderBottom': '1px solid #dee2e6'}),
            html.Td(str(location), style={'padding': '1rem', 'borderBottom': '1px solid #dee2e6'}),
            html.Td([
                detail_link
